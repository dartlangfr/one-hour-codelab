...
void  main() {
  ngBootstrap(module: new PirateBadgeModule());
}

class PirateBadgeModule extends Module {
  PirateBadgeModule() {
    type(PirateBadgeController);
  }
}

@NgController(selector: '[pirate-badge]', publishAs: 'ctrl')
class PirateBadgeController {
  bool dataLoaded = false;
  PirateName pirateBadge;
  
  PirateBadgeController() {
    PirateName.readyThePirates()
      .then((_) {
        //on success
        dataLoaded = true;
        pirateBadge = getBadgeNameFromStorage();
      })
      .catchError((arrr) {
  ...
  updateBadge(inputName) => setBadgeName(new PirateName(firstName: inputName));
  generateBadge() => setBadgeName(new PirateName());
  setBadgeName(PirateName newName) {
    pirateBadge = newName;
